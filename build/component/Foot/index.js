...
class FootComponent extends react_1.Component {
    ...
    onTransition(runtime) {
        ...
        let year = calculate(moment_1.default.duration(result).as('years'), 1);
        let day = calculate(moment_1.default.duration(result).as('days'), 365);
        let hours = calculate(moment_1.default.duration(result).as('hours'), 24);
        let minutes = calculate(moment_1.default.duration(result).as('minutes'), 60);
        let seconds = calculate(moment_1.default.duration(result).as('seconds'), 60);
        return `${year ? year + '年-' : ''}${day}天-${hours}小時-${minutes}分鐘-${seconds}秒`;
    }
    render() {
        let distance = this.onTransition(this.state.runtime);
        return (react_1.default.createElement("div", { className: "foot" },
            react_1.default.createElement("p", { className: "foot-inline-num" },
                "\u7576\u524D\u5728\u7DDA\u4EBA\u6578: ",
                this.state.online),
            react_1.default.createElement("p", { className: "foot-go-data" },
                "\u535A\u5BA2\u904B\u884C\u6642\u9577\uFF1A",
                react_1.default.createElement("span", null, distance),
                react_1.default.createElement("span", { className: "am-my-face" }, "(\u25CF'\u25E1'\u25CF)\uFF89\u2665")),
            react_1.default.createElement("p", { className: "foot-source" },
                "Made with ",
